Ext.namespace('prc.reminder.reminderviewall');

var winViewReminderDetails;
var winModifyReminder;

var hanViewAllReminders = function(btn)
{
	Ext.getCmp('tabHelp').getItem('tabitmHelp').load('../assets/e/prc/help/reminder-search.htm');
	
	Ext.getCmp('pbarPRC').updateProgress(0.3, 'Opening Reminder View All Screen', true);
	var tabCenter = Ext.getCmp('tabCenter');
	Ext.getCmp('pbarPRC').updateProgress(0.5, 'Initilizing Tab', true);
	
	hanViewAllReminderResult();
	
	if(Ext.getCmp('tabitmViewAllReminders'))
	{
		tabCenter.setActiveTab('tabitmViewAllReminders');
		return;
	}	
	
	tabCenter.add(
	{
		title: 'View All Remindersr',
		tabTip: 'View All Reminders',
		id: 'tabitmViewAllReminders',
		closable: true,
...
var hanViewAllReminderResult = function(btn)
{
	...
	storReminderViewAll.on('beforeload', function(store)
	{
		storReminderViewAll.baseParams = {subject: strReminderSubject, details: strReminderDetails, date: strReminderDate, startTime: strFromTime, endTime: strToTime };
	});
	
	storReminderViewAll.load({callback:function()
	{
		var intSearchResultCount = storReminderViewAll.getCount();
		...
		else
		{
			...
			{
				title: 'No Data',
				msg: "No reminder found for your search criteria",
				width: 300,
...
var hanModifyReminder = function(grid, rowIndex)
{
	...
	Ext.Ajax.request(
	{
		...
		failure: function()
		{
			...
			{
				title: 'Server Error',
				msg: "Some Error in the Sever, Please try again or contact Customer care",
				width: 300,
				buttons: Ext.MessageBox.OK,
	...
	function deletereminder()
	{
		Ext.Ajax.request(
		{
			...
			success: function(responseObject)
			{
				...
				storReminderViewAll.remove(rec);
			},
			failure: function()
			{
				Ext.Msg.show(
...
var storReminderViewAll = new Ext.data.JsonStore(
{
